(ns rtos.main
  "Kotodama WASM entrypoint for the Rtos clean-room actor (L4) — Clojure port.
  ...
  facts (`rtos.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"rtos.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rtos-compat" :tier tier :entities entities} 200])
